...
//
//	Author:			Corinnne Brucks
//
//  Latest update:	April 9th, 2015
//
...
{
	{ 0.8f, -0.5f, -0.75f, 400.00f },
	{ 0.8f, 0.5f, -0.75f, 400.00f },
	{ 0.8f, 0.0f, -0.500f, 400.00f },
...
const float BACKCOLOR[ ] = { 0.1, 0.1, 0.1, 0. };
...
Display( void )
{
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Corinne Brucks Assignment 2" );
	...
	for (int i = 0; i < NX; i++)
	{
		...
		for (int j = 0; j < NY; j++)
		{
			...
			for (int k = 0; k < NZ; k++)
			{
				...
				//Nodes[i][j][k].t = (0.8 * tempcalc + 20) / 100;
				Nodes[i][j][k].t = (0.8 * (tempcalc - TEMPMIN) + (TEMPMAX - TEMPMIN) * 0.2) / TEMPMAX ;
				glColor3f(Nodes[i][j][k].t, 0.0, 0.0);
