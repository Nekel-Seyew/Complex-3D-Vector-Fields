...
int WINAPI WinMain(HINSTANCE hinst, HINSTANCE, LPSTR, int)
{
	...
	basicVR.pRoomScene = new Scene(true); // <==============================================MODIFY DEFAULT?
    ...
    while (basicVR.HandleMessages())
    {
		...
		if (DIRECTX.Key['3']) ovrHmd_SetBool(basicVR.HMD, "QueueAheadEnabled", true);
		else                  ovrHmd_SetBool(basicVR.HMD, "QueueAheadEnabled", false);
        ...
        {
			basicVR.Layer[0]->RenderSceneToEyeBuffer(basicVR.MainCam, basicVR.pRoomScene, eye, 0, 0, timesToRenderRoom);
        }
