...
bool OculusWorldDemoApp::SetupWindowAndRendering(int argc, const char** argv)
{
    ...
    {
        SupportsSrgb = false;
        // Right now, ovrHmd_CreateSwapTextureSetGL cannot create MSAA textures.
    ...
    // The backbuffer is only used for the mirror window, not any of the HMD rendering,
    // and the blit won't convert to and from sRGB correctly, so we leave it non-sRGB
    RenderParams.SrgbBackBuffer = false;
...
void OculusWorldDemoApp::PopulateOptionMenu()
{
    ...
    */
    ...
    if (SupportsSrgb)
    {
        Menu.AddBool("Render Target.sRGB Eye Buffers 'F5'",   &SrgbRequested)       .AddShortcutKey(Key_F5).SetNotify(this, &OWD::RendertargetFormatChange);
    }
    ...
    Menu.AddEnum("Perf HUD.Mode 'F11'", &PerfHudMode).AddEnumValue("Off", ovrPerfHud_Off).
...
Sizei OculusWorldDemoApp::EnsureRendertargetAtLeastThisBig(int rtNum, Sizei requestedSize)
{
    ...
    bool depthFormatChanged = allocatedDepthFormat != desiredDepthFormat;
    ...
    if (currentSize != newRTSize || depthFormatChanged)
    {
        ...
        if (SupportsMultisampling && AllowMsaaTargets[rtNum])
        {
            ...
            if ( SupportsDepthMultisampling )
            {
                msrt.pDepthTex = *pRender->CreateTexture(desiredDepthFormat | msaaRate | Texture_SwapTextureSet, newRTSize.w, newRTSize.h, NULL);
        ...
        {
            rt.pDepthTex = *pRender->CreateTexture(desiredDepthFormat | Texture_SwapTextureSet, newRTSize.w, newRTSize.h, NULL);
...
void OculusWorldDemoApp::OnIdle()
{
    ...
    case MenuHudMove_FixedToFace:
        ...
        // Note we use ovrLayerType_QuadHeadLocked, so this isn't actually used by the rendering.
    ...
    if (bupdateRenderedView)
    {
        ...
        ovrHmd_GetEyePoses(Hmd, 0, eyeRenderHmdToEyeOffset, EyeRenderPose, nullptr);
    ...
    if ( LayersEnabled )
    {
        ...
        ovrLayerType menuLayerType = ovrLayerType_QuadHeadLocked;
        switch ( MenuHudMovementMode )
        {
        case MenuHudMove_FixedToFace:
            // Pos+orn are relative to the current HMD.
            menuLayerType = ovrLayerType_QuadHeadLocked;
        ...
        case MenuHudMove_DragAtEdge:
            ...
            menuLayerType = ovrLayerType_QuadInWorld;
            menuPose = MenuPose;
            break;
        default: OVR_ASSERT ( false ); break;
        }
        ...
        if ( HudRenderedSize.w > 0 )
        {
            ...
            HudLayer.Header.Type      = menuLayerType;
        ...
        if ( ( MenuRenderedSize.w > 0 ) && LayerHudMenuEnabled )
        {
            ...
            // Note the use of TextureOriginAtBottomLeft because these are rendertargets.
            MenuLayer.Header.Type      = menuLayerType;
                           ...
                           MirrorTexture->GetHeight() != WindowSize.h))
    {
        ...
        if ( RenderParams.SrgbBackBuffer )
        {
            // The mirror texture must match the backbuffer - the blit will not convert.
            format |= Texture_SRGB;
        }
        MirrorTexture = *pRender->CreateTexture(format, WindowSize.w, WindowSize.h, nullptr);
    }
...
{
    pPlatform->SetWindowSize(Hmd->Resolution.w, Hmd->Resolution.h);
